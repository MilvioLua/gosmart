...
//		if ($this->_firstColumnIndex > 255) $this->_firstColumnIndex = 255;
	...
	function close()
	{
		...
		for ($i = 0; $i <= $maxCol; ++$i) {
			...
			// the $firstcol first column on the range
			// the $lastcol  last column on the range
			// the $width	width to set
			// the $xfIndex  The optional cell style Xf index to apply to the columns
			// the $hidden   The optional hidden atribute
			// the $level	The optional outline level
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	private function _writeCFRule(PHPExcel_Style_Conditional $conditional){
		...
		// the $type : Type of the CF
		// the $operatorType : Comparison operator
		if($conditional->getConditionType() == PHPExcel_Style_Conditional::CONDITION_EXPRESSION){
			$type = 0x02;
			$operatorType = 0x00;
		...
		// the $szValue1 : size of the formula data for first value or formula
		// the $szValue2 : size of the formula data for second value or formula
