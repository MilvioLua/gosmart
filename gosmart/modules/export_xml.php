...
if ( $personal ) {
	...
	$x->writeElement( 'participantname', ( $survey_exp[ 'user_details' ]->name ? $survey_exp[ 'user_details' ]->name : esc_html__( 'Anonymous', MODAL_SURVEY_TEXT_DOMAIN ) ) );
	$x->writeElement( 'username', ( $survey_exp[ 'user_details' ]->username ? $survey_exp[ 'user_details' ]->username : esc_html__( 'Not Specified', MODAL_SURVEY_TEXT_DOMAIN ) ) );
	...
	if ( $survey_exp[ 'user_details' ]->alltimer > 0 ) {
		$x->writeElement( 'requiredtime', $survey_exp[ 'user_details' ]->alltimer . esc_html__( 'sec', MODAL_SURVEY_TEXT_DOMAIN ) );
	}
			...
			if ( is_numeric( $key ) ) {
				...
				if ( $personal ) {
					if ( in_array( $key, $user_votes[ $qkey ] ) ) {
						...
						$x->writeElement( 'admin_comment', $aoptions[ 16 ] );
					}
					else {
