...
if ( ! class_exists( 'manual_plugin_updater' ) ) {
	class manual_plugin_updater {
		...
		private function __construct( $pluginname, $newpluginname, $removedfiles = array(), $classname = "" ) {
			...
			switch ( $this->action ) {
				case 'upload':
					if ( ! wp_verify_nonce( $this->nonce, 'manual-plugin-updater-upload' ) ) $this->throw_error(esc_html__( 'Security check failed, Update process terminated.', 'manual_plugin_updater' ));
					break;
				case 'remove':
					if ( ! wp_verify_nonce( $this->nonce, 'manual-plugin-updater-remove' ) ) $this->throw_error(esc_html__( 'Security check failed, Update process terminated.', 'manual_plugin_updater' ),$this->temp_path);
					break;
				case 'update':
					if ( ! wp_verify_nonce( $this->nonce, 'manual-plugin-updater-update' ) ) $this->throw_error(esc_html__( 'Security check failed, Update process terminated.', 'manual_plugin_updater' ),$this->temp_path);
					break;
			}
			if ( ! $this->error ) {
				if ( $this->action == "upload" ) {
					if ( ! empty( $_FILES[ 'pluginupdate_file' ][ 'name' ] ) ) {
						...
						$this->throw_error( esc_html__( "File doesn't exists or damaged.", 'manual_plugin_updater' ) );
					}
					if ( isset( $this->uploaded[ 'error' ] ) ) {
						$this->throw_error( $this->uploaded[ 'error' ], 'manual_plugin_updater' );
					}
					if ( ! $this->error ) {
						print( '<p>' . esc_html__( 'ZIP Uploaded...', 'manual_plugin_updater' ) . '</p>');
		...
		private function unzip_update($zip)
		{
			...
			if(!file_exists($this->temp_path))
			{
				...
				{
					$this->throw_error(esc_html__( 'Failed to create folders...', 'manual_plugin_updater' ));
				}
			}
			$unzipfile = unzip_file( $zip['file'], $this->temp_path);
			   if ( $unzipfile ) {
				@unlink( $zip[ 'file' ] );
				print( '<p>' . esc_html__( 'ZIP unpacked, collecting datas...', 'manual_plugin_updater' ) . '</p>' );
				$newversion_path = plugin_dir_path( __FILE__ ) . "mpu-temp/" . $this->newpluginname;
					...
					if ( ! isset( $this->newplugindatas[ 'Name' ] ) && ! $this->error ) {
					   $this->throw_error( esc_html__( 'ZIP file is not compatible with this plugin or it is not a valid WordPress Plugin', 'manual_plugin_updater' ), $this->temp_path );
					}
				    if ( ! $this->error ) {
						if ( $this->plugindatas[ 'Name' ] != $this->newplugindatas[ 'Name' ] ) {
							$this->throw_error( esc_html__( 'Plugin name doesn\'t match. Is this the same plugin?', 'manual_plugin_updater' ), $this->temp_path );
						}
					}
				   if ( ! $this->error ) {
						$newdir = explode( '/', $this->newpluginname );
						$output = "<div class='manual-plugin-updater-update-informations'>";
						$output .= "<p><strong>" . esc_html__( 'Current Version: ', 'manual_plugin_updater' ) . "</strong>" . $this->plugindatas[ 'Version' ] . "</p><br><br>";
						$output .= "<p><strong>" . esc_html__( 'Uploaded Version: ', 'manual_plugin_updater' ) . "</strong>" . $this->newplugindatas[ 'Version' ] . "</p>";
						$output .= "</div>";
						$output .= "<div class='manual-plugin-updater-buttons'><form class='update_plugin_form' method='post'><input type='hidden' name='manual_plugin_updater' value='update'><input type='submit' class='button button-primary button-small' value='".esc_html__( 'START UPDATE', 'manual_plugin_updater' )."'><input type='hidden' name='nonce' value='".wp_create_nonce('manual-plugin-updater-update')."'></form><form class='update_plugin_form' method='post'><input type='hidden' name='manual_plugin_updater' value='remove'><input type='submit' class='button button-secondary button-small' value='".esc_html__( 'DO NOT UPDATE (Delete uploaded temp files)', 'manual_plugin_updater' )."'><input type='hidden' name='nonce' value='".wp_create_nonce('manual-plugin-updater-remove')."'></form></div>";
						if ( file_exists( str_replace( $newdir[ 1 ], '', $newversion_path ) . 'changelog.txt' ) ) {
							...
							$output .= "<div class='manual-plugin-updater-changelog'><h3>" . esc_html__( 'Changelog', 'manual_plugin_updater' ) . "</h3>" . nl2br( $changelog ) . "</div>";
						}
						return print( nl2br( $output ) );
					}
			   } else $this->throw_error(esc_html__( 'There was an error unzipping the file. Is it a valid ZIP file?', 'manual_plugin_updater' ),$this->temp_path);
		}
		...
		private function remove() {
			...
			$this->update_screen();
			print( "<p>" . esc_html__( 'Update Files has been successfully removed.', 'manual_plugin_updater' ) . "</p>" );
		}
		
		private function update() {
			$newdir = explode( '/', $this->pluginname );
			print("<p>" . esc_html__( 'Copying files...', 'manual_plugin_updater' ) . "</p>");
			if ( $this->copy_files( $this->temp_path . '/' . $newdir[ 0 ], WP_PLUGIN_DIR . '/' . $newdir[ 0 ] ) ) {
				$this->erasedir($this->temp_path);
			}
			print( "<p>" . esc_html__( 'Removing temp folder...', 'manual_plugin_updater' ) . "</p>");
			if ( ! empty( $this->removedfiles ) ) {
				$this->delete_files( WP_PLUGIN_DIR . '/' . $newdir[ 0 ], $this->removedfiles );
			}
			print( "<p>" . esc_html__( 'Plugin files has been successfully updated...', 'manual_plugin_updater' ) . "</p>" );
			if ( $this->classname == "" ) {
			...
			if ( class_exists( $this->classname ) ) {
				...
				print( "<p>" . esc_html__( 'Plugin Activation Initialized Successfully...', 'manual_plugin_updater' ) . "</p>");
			}
		}
		
		private function update_screen() {
			print('<p>' . esc_html__( 'After uploading the ZIP file you can decide to continue the update process or discard the changes.', 'manual_plugin_updater' ) . '</p>
			...
			' . esc_html__( 'Browse the new version ZIP file:', 'manual_plugin_updater' ) . ' <input type="file" name="pluginupdate_file">
			<input type="submit" class="button button-secondary button-small" class="update_plugin" value="' . esc_html__( 'UPLOAD', 'manual_plugin_updater' ) . '">
			</form>');
		}
