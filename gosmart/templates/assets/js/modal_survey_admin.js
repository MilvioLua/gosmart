(function( jQuery ){
	...
	var rmdni = false, exportcharts = 2, thisexpelement = '', actionfl = false,	survey_id, lastScrollTop = 0, cond_divider = "", exploded_advcond = [], remocounter = 0, cond_statement = "", surveysystem = jQuery.noConflict(), lastcondset = "", lastlength = 0, buttonspan_global = "", tempstoragehtml, tempstorage, active_survey, random, played_question = 0, delprsurv = "", exporttimer, ziptimer, msChartData = {}, labs = [], dset = [], msChartBGcolor = [], msChartBGcolor2 = [], handler = 0, selectanswer_values = '';
	...
	console.log(sspa_params);
     surveysystem( "#dialog-confirm12" ).dialog({
      resizable: false,
      height: 180,
	  show: {
			effect: "fade",
      ...
      buttons: [
		{
			text: sspa_params.languages.yes,
			click : function() {
				surveysystem( ".ms-wizard" ).trigger( "click" );
	...
	if ( sspa_params.status == 'new' ) {
		setTimeout( function() {
			surveysystem( "#dialog-confirm12" ).dialog( "open" );
		}, 3000 );
	}
	...
	//handle advanced conditions on the survey edit screen with AND
	surveysystem( document ).on( "click", ".add_condition_advanced", function(event) {
		event.preventDefault();
		tempstorage = surveysystem( ".temp-condition-storage" );
		tempstoragehtml = surveysystem( ".temp-condition-storage" ).html();
		if ( surveysystem( ".conds" ).val() != "time" && surveysystem( ".condvalue" ).val() == "" ) {
			showmessage( sspa_params.languages.conditionsmust );
		}
		else {
			if ( surveysystem( ".conds" ).val() == "time" ) {
				if ( tempstoragehtml.indexOf( "time" ) > 0 ) {
					
				}
				else {
					if ( tempstoragehtml.substr( tempstoragehtml.length - 4 ) == "AND" || tempstoragehtml.substr( tempstorage.length - 3 ) == "OR" || surveysystem( ".temp-condition-storage" ).html() == "" ) {
						tempstorage.html( surveysystem( ".temp-condition-storage" ).html() + " " + surveysystem( ".conds" ).val() );
						lastcondset = " " + surveysystem( ".conds" ).val();
					}
					else {
						showmessage( sspa_params.languages.conditionmissingandor );
					}
				}
			}
			else {
				if ( tempstoragehtml.substr( tempstoragehtml.length - 4 ).trim() == "AND" || tempstoragehtml.substr( tempstorage.length - 3 ).trim() == "OR" || surveysystem( ".temp-condition-storage" ).html() == "" ) {
					tempstorage.html( tempstorage.html() + " " + surveysystem( ".conds" ).val() + " " + surveysystem( ".relation" ).val() + " " + surveysystem( ".condvalue" ).val() );
					lastcondset = " " + surveysystem( ".conds" ).val() + " " + surveysystem( ".relation" ).val() + " " + surveysystem( ".condvalue" ).val();
				}
				else {
						showmessage( sspa_params.languages.conditionmissingandor );					
				}
		...
		if ( surveysystem( ".temp-condition-storage" ).html() != "" && surveysystem( ".temp-condition-storage .remove_temp_conditions" ).length == 0 ) {
			tempstorage.prepend( '<a href="#" class="remove_temp_conditions"><img class="modal_survey_tooltip" title="Remove Condition" src="http://dev.pantherius.com/wp-content/plugins/modal_survey/templates/assets/img/icon-minus.png"></a>');
		}
	});
	
	//handle advanced conditions on the survey edit screen with AND
	surveysystem( document ).on( "click", ".add_condition_and", function(event) {
		event.preventDefault();
		tempstorage = surveysystem( ".temp-condition-storage" );
		tempstoragehtml = surveysystem( ".temp-condition-storage" ).html();
		if ( tempstoragehtml.substr( tempstoragehtml.length - 4 ).trim() == "AND" || tempstoragehtml.substr( tempstorage.length - 3 ).trim() == "OR" ) {
			showmessage( sspa_params.languages.conditionalreadyhasandor );
		}
		else {
			if ( tempstoragehtml != "" && tempstoragehtml.indexOf( " OR" ) == -1 ) {
				surveysystem( ".temp-condition-storage" ).html( tempstoragehtml + " AND" );
				lastcondset = " AND";
			}
			else {
				showmessage( sspa_params.languages.conditionemptyorconflict );
			}
		}
	});
	
	//handle advanced conditions on the survey edit screen with OR
	surveysystem( document ).on( "click", ".add_condition_or", function(event) {
		event.preventDefault();
		tempstorage = surveysystem( ".temp-condition-storage" );
		tempstoragehtml = surveysystem( ".temp-condition-storage" ).html();
		if ( tempstoragehtml.substr( tempstoragehtml.length - 4 ).trim() == "AND" || tempstoragehtml.substr( tempstorage.length - 3 ).trim() == "OR" ) {
			...
			if ( tempstoragehtml != "" && tempstoragehtml.indexOf( " AND" ) == -1 ) {
				surveysystem( ".temp-condition-storage" ).html( tempstoragehtml + " OR" );
				lastcondset = " OR";
			}
			else {
				showmessage( sspa_params.languages.conditionemptyorconflict );
	...
	//remove the last added condition to the advanced conditions set
	surveysystem( document ).on( "click", ".remove_lastcondition", function(event) {
		event.preventDefault();
		lastlength = - Math.abs( lastcondset.length );
		if ( lastlength != 0 ) {
			surveysystem( ".temp-condition-storage" ).html( surveysystem( ".temp-condition-storage" ).html().slice( 0, lastlength ) );
			if ( surveysystem( ".temp-condition-storage" ).html() == '<a href="#" class="remove_temp_conditions"><img class="modal_survey_tooltip" title="Remove Condition" src="http://dev.pantherius.com/wp-content/plugins/modal_survey/templates/assets/img/icon-minus.png"></a>' ) {
				surveysystem( ".temp-condition-storage" ).html( "" );
			}
		}
		lastcondset = "";
	});
		
	//clear temp conditions storage
	surveysystem( document ).on( "click", ".remove_temp_conditions", function(event) {
		event.preventDefault();
		surveysystem( ".temp-condition-storage" ).html( "" );
	});
	...
	surveysystem( document ).on( "click", ".default-settings", function(event) {
		event.preventDefault();
		surveysystem( "#dialog-confirm11" ).dialog( "open" );
	})
	...
	surveysystem( document ).on( "click", ".add_condition", function( event ) {
		...
		if ( surveysystem( ".temp-condition-storage" ).html() != "") {
			if ( surveysystem( ".actionvalue" ).val() == "" ) {
				showmessage( sspa_params.languages.conditionsmust );
				return;
			}
			else {
				var advcond = surveysystem( ".temp-condition-storage" ).html().replace(/\AND/g,"||").replace(/\OR/g,"&&").replace(/\equal/g,"=").replace(/\higher/g,">").replace(/\lower/g,"<").replace( '<a href="#" class="remove_temp_conditions"><img class="modal_survey_tooltip" title="Remove Condition" src="http://dev.pantherius.com/wp-content/plugins/modal_survey/templates/assets/img/icon-minus.png"></a>', '' ).replace(/\s/g,"");
				if ( advcond.indexOf( "||" ) > 0 ) {
					cond_divider = "||";
				}
				if ( advcond.indexOf( "&&" ) > 0 ) {
					cond_divider = "&&";
				}		
				var adv_cond_list = advcond.split( cond_divider );
				var final_adv_cond1 =  "";
				var final_adv_cond2 =  "";
				var final_adv_cond3 =  "";
				surveysystem.each( adv_cond_list, function( i, v ) {
					if ( v.indexOf( "=" ) > 0 ) {
						cond_statement = "=";
					}
					if ( v.indexOf( ">" ) > 0 ) {
					...
					exploded_advcond = v.split( cond_statement );
					final_adv_cond1 += exploded_advcond[ 0 ];
					final_adv_cond2 += cond_statement.replace( ">", "higher" ).replace( "<", "lower" ).replace( "=", "equal" );
					final_adv_cond3 += exploded_advcond[ 1 ];
					if ( adv_cond_list.length - 1 > i ) {
						final_adv_cond1 += cond_divider;
						final_adv_cond2 += cond_divider;
						final_adv_cond3 += cond_divider;
					}
				})
					 newline = [ 
						final_adv_cond1,
		...
		if ( newline[ 0 ].indexOf( cond_divider ) > 0 ) {
			var expl_conds_list = newline[ 0 ].split( cond_divider );
		}
		else {
			var expl_conds_list = [
			newline[ 0 ]
		...
		surveysystem.each( expl_conds_list, function( i, nline ) {
			if ( nline == "time" ) {
				newline_str += sspa_params.languages.finaltime + " ";
			}
			if ( nline == "score" ) {
				newline_str += sspa_params.languages.finalscore + " ";
			}
			if ( nline == "avgscore" ) {
				newline_str += sspa_params.languages.avgscore + " ";
			}
			if ( nline == "correct" ) {
				newline_str += sspa_params.languages.correctanswers + " ";
			}
			if ( nline.indexOf( "questionscore_" ) >= 0 ) {
				var qsexp = nline.split( "_" );
				newline_str += sspa_params.languages.question + " " + qsexp[ 1 ] + " " + sspa_params.languages.score + " ";
			}
			if ( nline.indexOf( "questionanswer_" ) >= 0 ) {
				var qsexp = nline.split( "_" );
				newline_str += sspa_params.languages.question + " " + qsexp[ 1 ] + " " + sspa_params.languages.answer + " ";
			}
			...
			if ( nline != "time" ) {
				if ( newline[ 1 ].indexOf( cond_divider ) > 0 ) {
					var expl_conds_list2 = newline[ 1 ].split( cond_divider );
				}
				else {
				...
				if ( newline[ 2 ].indexOf( cond_divider ) > 0 ) {
					var expl_conds_list3 = newline[ 2 ].split( cond_divider );
				}
				else {
				...
				if ( expl_conds_list2[ i ] == "higher" ) newline_str += sspa_params.languages.higherthan + " ";
				if ( expl_conds_list2[ i ] == "equal" ) newline_str += sspa_params.languages.equalwith + " ";
				if ( expl_conds_list2[ i ] == "lower" ) newline_str += sspa_params.languages.lowerthan + " ";
				newline_str += expl_conds_list3[ i ];
			}
			if ( expl_conds_list.length - 1 > i ) {
				if ( cond_divider == "||" ) {
					newline_str += " AND ";
				}
				if ( cond_divider == "&&" ) {
	...
	surveysystem( document ).on( "click", ".edit_condition", function( event ) {
		...
		if ( conds_array[ 0 ].indexOf( "||" ) > 0 || conds_array[ 0 ].indexOf( "&&" ) > 0 ) {
			showmessage( sspa_params.languages.advcondblockedit );
			return;
		}
	...
	function isUrlValid( url ) {
		return /^(https?|s?ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?((\[)?([a-zA-Z0-9]){1,}(\])?)?$/i.test( url );
	}
	...
	surveysystem( ".survey_preloader" ).on( "change", function() {
		surveysystem( "#preloader_preview img" ).attr( "src", sspa_params.plugin_url + "/templates/assets/img/" + surveysystem( this ).val() + ".gif" );
	})

	surveysystem( ".survey_closeicon" ).on( "change", function() {
		surveysystem( "#closeicon_preview img" ).attr( "src", sspa_params.plugin_url + "/templates/assets/img/" + surveysystem( this ).val() + ".png" );
	})

	surveysystem( ".survey_theme" ).on( "change", function() {
		surveysystem( "#theme_preview img" ).attr( "src", sspa_params.plugin_url + "/templates/assets/img/theme_preview" + surveysystem( this ).val() + ".jpg" );
		if ( surveysystem( this ).val() != "" ) {
			surveysystem( ".style-theme-specific" ).addClass( "dnone" );
		}
		else {
			surveysystem( ".style-theme-specific" ).removeClass( "dnone" );			
		}
	})
	...
	function play_survey() {
		...
		var thiscount = surveysystem("#"+survey_id+" #"+thisquestion+" div span#answer_count"+(index2+1)).text().split(" - ");
			...
			numeric_min = "0";
			if ( surveysystem( this ).parent().find( ".ao-accordion .numeric_answer_min" ).length > 0 ) {
				numeric_min = surveysystem( this ).parent().find( ".ao-accordion .numeric_answer_min" ).val();;
			}
			numeric_max = "100";
			if ( surveysystem( this ).parent().find( ".ao-accordion .numeric_answer_max" ).length > 0 ) {
				numeric_max = surveysystem( this ).parent().find( ".ao-accordion .numeric_answer_max" ).val();;
			}
			numeric_step = "1";
			if ( surveysystem( this ).parent().find( ".ao-accordion .numeric_answer_step" ).length > 0 ) {
				numeric_step = surveysystem( this ).parent().find( ".ao-accordion .numeric_answer_step" ).val();;
			}
			selectanswer_values = '';
			if ( surveysystem( this ).parent().find( ".ao-accordion .selectanswer_values" ).length > 0 ) {
				selectanswer_values = surveysystem( this ).parent().find( ".ao-accordion .selectanswer_values" ).val();
			}
			ao[ ( index + 1 ) + "_" + ( index2 + 1 ) ] = [answer_type, answer_unique, autocomplete, imageurl, score, correct, imgwidth, imgheight, answer_status, converttextarea, surveysystem( this ).parent().find( ".ao-accordion .answer_tooltip_text" ).val(), surveysystem( this ).parent().find( ".ao-accordion .answer_redirection" ).val(), surveysystem( this ).parent().find( ".ao-accordion .answer_category" ).val(), hidelabel, surveysystem( this ).parent().find( ".ao-accordion .img-align:checked" ).val(), surveysystem( this ).parent().find( ".ao-accordion .answer_tooltip_text" ).val(), surveysystem( this ).parent().find( ".ao-accordion .answer_comment_text" ).val(), selectanswer_values, numeric_min, numeric_max, numeric_step];
			})
			qoptions.push( [ surveysystem( "#" + survey_id + " #" + thisquestion + " input.choices" ).val(), surveysystem( "#" + survey_id + " #" + thisquestion + " input.minchoices" ).val(), qimageurl, surveysystem( "#" + survey_id + " #" + thisquestion + " .qopts_rating" ).val(), surveysystem( "#" + survey_id + " #" + thisquestion + " .question_tooltip_text" ).val(), surveysystem( "#" + survey_id + " #" + thisquestion + " .question_category" ).val(), surveysystem( "#" + survey_id + " #" + thisquestion + " .qopts_width" ).val(), surveysystem( "#" + survey_id + " #" + thisquestion + " .qopts_height" ).val(), surveysystem( "#" + survey_id + " #" + thisquestion + " .img-align:checked" ).val(), surveysystem( "#" + survey_id + " #" + thisquestion + " .section_content_text" ).val() ] );
			thissurvey.push(qa);
		})
		aoptions.push( ao );
		...
		lang_array.back_button = sspa_params.languages.back_button;
		lang_array.next_button = sspa_params.languages.next_button;
		var msurvey_admin_options = JSON.stringify({
			...
			surveysystem( "#" + survey_id + " .allconditions" ).val(),
			surveysystem( "#" + survey_id + " .show_admin_comments" ).val(),
			surveysystem( "#" + survey_id + " .addmoreapi_mailingboss" ).val(),
			surveysystem( "#" + survey_id + " .mailingboss_key" ).val(),
			surveysystem( "#" + survey_id + " .mailingboss_listid" ).val(),
			surveysystem( "#" + survey_id + " .addmoreapi_klicktipp" ).val(),
			surveysystem( "#" + survey_id + " .klicktipp_username" ).val(),
			surveysystem( "#" + survey_id + " .klicktipp_password" ).val(),
			surveysystem( "#" + survey_id + " .klicktipp_apikey" ).val(),
			surveysystem( "#" + survey_id + " .survey_theme" ).val()
			]),
	...
	surveysystem( "body" ).on( "click", ".play_button", function() {
		if ( handler == 0 ) {
			...
			if ( surveysystem( "#" + active_survey + " .lock_bg" ).val() == 1 ) {
				if ( surveysystem( "#bglock" ).length == 0 ) {
					surveysystem( "body" ).append( "<div id='bglock'></div>" );
				}
				surveysystem( "#bglock" ).fadeIn( 1000 );
			}
			play_survey();
			surveysystem( ".floating-play-button span" ).text( sspa_params.languages.close_survey );
			surveysystem( ".floating-play-button img" ).attr( "src" , surveysystem( ".floating-play-button img" ).attr( "src" ).replace( "play", "close" ) );
			surveysystem( ".floating-play-button" ).removeClass( "floating-play-button" ).addClass( "floating-close-button" );
		}
		handler = 0;
	})

	surveysystem( "body" ).on( "click", "#close_survey", function() {
		surveysystem( ".floating-close-button span" ).text( sspa_params.languages.play_survey );
		surveysystem( ".floating-close-button img" ).attr( "src" , surveysystem( ".floating-close-button img" ).attr( "src" ).replace( "close", "play" ) );
		surveysystem( ".floating-close-button" ).removeClass( "floating-close-button" ).addClass( "floating-play-button" );
		handler = 1;
	})
	
	surveysystem( "body" ).on( "click", ".floating-close-button", function() {
		surveysystem( "#survey-" + random + "-1" ).modalsurvey( "close" );
		surveysystem( "#close_survey" ).trigger( "click" );
		if ( surveysystem( ".floating-close-button" ).length > 0 ) {
			surveysystem( ".floating-close-button span" ).text( sspa_params.languages.play_survey );
			surveysystem( ".floating-close-button img" ).attr( "src" , surveysystem( ".floating-close-button img" ).attr( "src" ).replace( "close", "play" ) );
			surveysystem( ".floating-close-button" ).removeClass( "floating-close-button" ).addClass( "floating-play-button" );
		}
...
function initialize_question_accordions(survey_id)
{
	...
	window.numbersandminusonly = function( e ){ 
		...
		if ( unicode != 8 ) { //if the key isn't the backspace key (which we should allow)
			if ( ( unicode < 48 || unicode > 57 ) && ( unicode != 45 )  && ( unicode != 46 ) ) {//if not a number
	...
	function remove_condition( cond ) {
		surveysystem( "a[data-cond='" + cond + "']" ).parent().remove(); 
		//surveysystem( ".one_condition_line" ).eq( cond ).remove();		
	}

	function default_options() {
		surveysystem( ".display_style, .survey_mode, .animation_type" ).prop( "selectedIndex", 0 );
		surveysystem( ".animation_easing" ).prop( "selectedIndex", 28 );
		surveysystem( ".modal_survey_animation_speed" ).slider( 'value', 0.5 );
		surveysystem( ".modal_survey_animation_speed_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_animation_speed_value" ).val().replace( /[^\d.]/g, '' ), '0.5' );
				});
		surveysystem( ".modal_survey_end_delay" ).slider( 'value', 3 );
		surveysystem( ".modal_survey_end_delay_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_end_delay_value" ).val().replace( /[^\d.]/g, '' ), '3' );
				});
		surveysystem( ".modal_survey_display_timer" ).slider( 'value', 0 );
		surveysystem( ".modal_survey_display_timer_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_display_timer_value" ).val().replace( /[^\d.]/g, '' ), '0' );
				});
		surveysystem( ".modal_survey_quiz_timer" ).slider( 'value', 0 );
		surveysystem( ".modal_survey_quiz_timer_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_quiz_timer_value" ).val().replace( /[^\d.]/g, '' ), '0' );
				});
		surveysystem( ".modal_survey_shadowh" ).slider( 'value', 0 );
		surveysystem( ".modal_survey_shadowh_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_shadowh_value" ).val().replace( /[^\d.]/g, '' ), '0' );
				});
		surveysystem( ".modal_survey_shadowv" ).slider( 'value', 1 );
		surveysystem( ".modal_survey_shadowv_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_shadowv_value" ).val().replace( /[^\d.]/g, '' ), '1' );
				});
		surveysystem( ".modal_survey_shadowb" ).slider( 'value', 1 );
		surveysystem( ".modal_survey_shadowb_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_shadowb_value" ).val().replace( /[^\d.]/g, '' ), '1' );
				});
		surveysystem( ".modal_survey_shadows" ).slider( 'value', 0 );
		surveysystem( ".modal_survey_shadows_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_shadows_value" ).val().replace( /[^\d.]/g, '' ), '0' );
				});
		surveysystem( ".modal_survey_border_width" ).slider( 'value', 1 );
		surveysystem( ".modal_survey_border_width_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_border_width_value" ).val().replace( /[^\d.]/g, '' ), '1' );
				});
		surveysystem( ".modal_survey_border_radius" ).slider( 'value', 3 );
		surveysystem( ".modal_survey_border_radius_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_border_radius_value" ).val().replace( /[^\d.]/g, '' ), '3' );
				});
		surveysystem( ".modal_survey_line_height" ).slider( 'value', 16 );
		surveysystem( ".modal_survey_line_height_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_line_height_value" ).val().replace( /[^\d.]/g, '' ), '16' );
				});
		surveysystem( ".modal_survey_padding" ).slider( 'value', 20 );
		surveysystem( ".modal_survey_padding_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_padding_value" ).val().replace( /[^\d.]/g, '' ), '20' );
				});
		surveysystem( ".modal_survey_font_size" ).slider( 'value', 16 );
		surveysystem( ".modal_survey_font_size_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_font_size_value" ).val().replace( /[^\d.]/g, '' ), '16' );
				});
		surveysystem( ".modal_survey_cookie_expiration" ).slider( 'value', 9999 );
		surveysystem( ".modal_survey_cookie_expiration_value" ).val( function( index, value ) {
				   return value.replace( surveysystem( ".modal_survey_cookie_expiration_value" ).val().replace( /[^\d.]/g, '' ), '9999' );
				});
		surveysystem( ".global_survey, .lock_bg, .closeable, .atbottom, .loggedin, .loggedin, .remandcont, .qtimer, .show_admin_comments, .progressbar, .listlayout, .alwaysnext, .enableback" ).prop( "checked", false );
		surveysystem( ".global_survey, .lock_bg, .closeable, .atbottom, .loggedin, .loggedin, .remandcont, .qtimer, .show_admin_comments, .progressbar, .listlayout, .alwaysnext, .enableback" ).val( "0" );
		surveysystem( ".survey_theme, .font_family, .grid_items, .next_button_style, .rating_question_style, .survey_preloader, .survey_hover, .survey_closeicon, .survey_closeiconsize" ).prop( "selectedIndex", 0 );
		surveysystem( ".text_align" ).prop( "selectedIndex", 1 );
		surveysystem( ".start_time, .expiry_time, .redirecturl, .notificationemail" ).val( "" );
		surveysystem( ".thankyou textarea" ).val( "Thank you for your feedback!" );
		surveysystem( ".survey_preloader" ).trigger( "change" );
		surveysystem( ".survey_closeicon" ).trigger( "change" );
		surveysystem( ".survey_theme" ).trigger( "change" );
		surveysystem( ".modal_survey_preview1001 .inner input" ).val( "-webkit-linear-gradient(top , rgb(244, 244, 244) 97% , rgb(244, 244, 244) 100%);background:-moz-linear-gradient(top , rgb(244, 244, 244) 97% , rgb(244, 244, 244) 100%);background:-ms-linear-gradient(top , rgb(244, 244, 244) 97% , rgb(244, 244, 244) 100%);background:-o-linear-gradient(top , rgb(244, 244, 244) 97% , rgb(244, 244, 244) 100%);background:linear-gradient(top , rgb(244, 244, 244) 97% , rgb(244, 244, 244) 100%)" );
		surveysystem( ".modal_survey_preview1001 .inner" ).css( "background", "" );
		surveysystem( ".modal_survey_preview1002" ).css( "background-color", "rgb(31, 31, 31)" );
		surveysystem( ".modal_survey_preview1003" ).css( "background-color", "rgb(206, 196, 196)" );
		surveysystem( ".modal_survey_preview1004" ).css( "background-color", "#ececec" );		
	}	
	...
	surveysystem( "#" + s_id + " .custom_field_section" ).append( "<div class='one-custom-field custom_field_section_html' data-type='html' data-id='" + unihtmlid + "'><textarea data-type='html' class='cfid' id='customhtml_" + unihtmlid + "'></textarea><div class='emptycheckbox'></div><img class='remove_cfield modal_survey_tooltip' title='" + sspa_params.languages.customhtml_remove + "' src='"+sspa_params.plugin_url+"/templates/assets/img/icon-minus.png'><div class='customhtml-tip'>" + sspa_params.languages.customhtml_tip + "</div><div class='customhtml_position'><label><input type='radio' name='customhtml_pos_" + unihtmlid + "' class='customhtml_pos' value='1'>" + sspa_params.languages.customhtml_pos1 + "</label><label><input type='radio' name='customhtml_pos_" + unihtmlid + "' class='customhtml_pos' checked value='2'>" + sspa_params.languages.customhtml_pos2 + "</label><label><input type='radio' name='customhtml_pos_" + unihtmlid + "' class='customhtml_pos' value='3'>" + sspa_params.languages.customhtml_pos3 + "</label></div></div>" );
 	...
 	function add_numeric_answer( thisanswer ) {
		var answer_area = surveysystem( thisanswer ).parent().parent().attr( "id" );
		var question_num = surveysystem( thisanswer ).parent().parent().parent().attr( "id" ).replace( "question_", "" );
		var answer_num = surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num + " .answer" ).length + 1;
		/*if ( surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num + " .numeric_answer_style").length > 0 ) {
			showmessage( sspa_params.languages.alreadynuma );
			return;
		}*/
		open_unique_id = ( Math.floor( Math.random() * 26 ) + Date.now() );
		surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num ).append( '<div class="added_answers" id="answer_element_' + answer_area.replace( "answers_", "" ) + '_' + answer_num + '"><a href="#" class="answer_options" data-aid="' + answer_num + '"><img class="modal_survey_tooltip" title="' + sspa_params.languages.answeroptions + '" src="' + sspa_params.plugin_url + '/templates/assets/img/options.png"></a><a href="#" class="answer_status answer_status' + answer_num + '" data-aid="' + answer_num + '"><img class="modal_survey_tooltip" title="' + sspa_params.languages.activeanswer + '" src="' + sspa_params.plugin_url + '/templates/assets/img/active.png"></a><span class="ans_nmo">'+answer_num+'.</span> ' + sspa_params.languages.answer + ': <input type="text" data-answertype="numeric" data-unique="' + open_unique_id + '" id="answer'+answer_num+'" name="answer[]" class="answer numeric_answer_style" value="' + sspa_params.languages.selectnum + '" /><span id="answer_count'+answer_num+'" class="answer_count" contenteditable="true">0 - 0%</span><a class="remove_answer" id="remove_'+answer_area+'_'+answer_num+'"><img class="modal_survey_tooltip" title="' + sspa_params.languages.removeanswer + '" src="'+sspa_params.plugin_url+'/templates/assets/img/icon-minus.png"></a><div class="ao-accordion oaa' + answer_num + ' oaa' + answer_area.replace( "answers_", "" ) + '_' + answer_num + '"><h4>' + sspa_params.languages.answeroptions + '</h4><div><div class="answer_title">' + sspa_params.languages.answer_numeric_title + '</div><div class="left_aopts"><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.imgwidthhint + '">' + sspa_params.languages.imgwidth + ': <input type="text" size="10" maxlength="10" placeholder="' + sspa_params.languages.imgwidthp + '" class="aopts_width optsinput" value=""></div><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.imgheighthint + '">' + sspa_params.languages.imgheight + ': <input type="text" size="10" maxlength="10" placeholder="' + sspa_params.languages.imgheightp + '" class="aopts_height optsinput" value=""></div><div class="imageelement"><div class="uploaded_image"><input class="answer-image-upload button add-button" type="button" value="' + sspa_params.languages.addimage + '" /></div></div><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.hidelabel_hint + '"><label><input type="checkbox" class="aopts_hide_label ms-checkbox" value=""><span>' + sspa_params.languages.hidelabel + '</span></label></div><div class="img-pos-cont"><div class="optscontainer">' + sspa_params.languages.imgpos + '</div><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" checked class="img-align" value=""> ' + sspa_params.languages.imgdef + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="1"> ' + sspa_params.languages.imgtop + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="2"> ' + sspa_params.languages.imgbot + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="3"> ' + sspa_params.languages.imgontop + '</label></div></div><div class="right_aopts"><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.scorehint + '">' + sspa_params.languages.answerscore + ': <input type="number" size="10" maxlength="10" onkeypress="return numbersandminusonly(event)" placeholder="' + sspa_params.languages.number + '" class="aopts_score optsinput" value="1"></div><div class="answer_tooltip optscontainer"><span>' + sspa_params.languages.customtooltip + '</span><textarea maxlength="300" class="answer_tooltip_text" placeholder="' + sspa_params.languages.customtooltipph + '"></textarea></div><div class="answer_redirection_container optscontainer modal_survey_tooltip" title="' + sspa_params.languages.redirecttooltip + '"><span>' + sspa_params.languages.clredirection + '</span><input class="answer_redirection optsinput" placeholder="' + sspa_params.languages.redplaceholder + '" onkeypress="return numbersandminusonly(event)" value=""></div><div class="answer_category_container optscontainer modal_survey_tooltip" title="' + sspa_params.languages.category_tooltip + '"><span>' + sspa_params.languages.category + '</span><input class="answer_category optsinput" placeholder="' + sspa_params.languages.category_placeholder + '" value=""></div><div class="optscontainer"><span>' + sspa_params.languages.minvalue + '</span><input type="number" onkeypress="return numbersandminusonly(event)" class="numeric_answer_min optsinput" value="0"></div><div class="select-values optscontainer"><span>' + sspa_params.languages.maxvalue + '</span><input type="number" onkeypress="return numbersandminusonly(event)" class="numeric_answer_max optsinput" value="100"></div><div class="select-values optscontainer"><span>' + sspa_params.languages.stepvalue + '</span><input type="number" onkeypress="return numbersandminusonly(event)" class="numeric_answer_step optsinput" value="1"></div><input type="hidden" class="aopts_status" value="0"></div></div></div>');
		create_graph( answer_area.replace( "answers_", "" ), question_num, "true" );
		initialize_tooltips();
		//surveysystem( "#answer_element_" + answer_area.replace( "answers_", "" ) + "_" + answer_num + " input" ).focus();
		surveysystem( ".oaa" + answer_area.replace( "answers_", "" ) + "_" + answer_num ).accordion({
 	...
 	function add_date_answer( thisanswer ) {
		var answer_area = surveysystem( thisanswer ).parent().parent().attr( "id" );
		var question_num = surveysystem( thisanswer ).parent().parent().parent().attr( "id" ).replace( "question_", "" );
		var answer_num = surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num + " .answer" ).length + 1;
		/*if ( surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num + " .date_answer_style").length > 0 ) {
			showmessage( sspa_params.languages.alreadydatea );
			return;
		}*/
		open_unique_id = ( Math.floor( Math.random() * 26 ) + Date.now() );
		surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num ).append( '<div class="added_answers" id="answer_element_' + answer_area.replace( "answers_", "" ) + '_' + answer_num + '"><a href="#" class="answer_options" data-aid="' + answer_num + '"><img class="modal_survey_tooltip" title="' + sspa_params.languages.answeroptions + '" src="' + sspa_params.plugin_url + '/templates/assets/img/options.png"></a><a href="#" class="answer_status answer_status' + answer_num + '" data-aid="' + answer_num + '"><img class="modal_survey_tooltip" title="' + sspa_params.languages.activeanswer + '" src="' + sspa_params.plugin_url + '/templates/assets/img/active.png"></a><span class="ans_nmo">'+answer_num+'.</span> ' + sspa_params.languages.answer + ': <input type="text" data-answertype="date" data-unique="' + open_unique_id + '" id="answer'+answer_num+'" name="answer[]" class="answer date_answer_style" value="' + sspa_params.languages.selectdate + '" /><span id="answer_count'+answer_num+'" class="answer_count" contenteditable="true">0 - 0%</span><a class="remove_answer" id="remove_'+answer_area+'_'+answer_num+'"><img class="modal_survey_tooltip" title="' + sspa_params.languages.removeanswer + '" src="'+sspa_params.plugin_url+'/templates/assets/img/icon-minus.png"></a><div class="ao-accordion oaa' + answer_num + ' oaa' + answer_area.replace( "answers_", "" ) + '_' + answer_num + '"><h4>' + sspa_params.languages.answeroptions + '</h4><div><div class="answer_title">' + sspa_params.languages.answer_date_title + '</div><div class="left_aopts"><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.imgwidthhint + '">' + sspa_params.languages.imgwidth + ': <input type="text" size="10" maxlength="10" placeholder="' + sspa_params.languages.imgwidthp + '" class="aopts_width optsinput" value=""></div><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.imgheighthint + '">' + sspa_params.languages.imgheight + ': <input type="text" size="10" maxlength="10" placeholder="' + sspa_params.languages.imgheightp + '" class="aopts_height optsinput" value=""></div><div class="imageelement"><div class="uploaded_image"><input class="answer-image-upload button add-button" type="button" value="' + sspa_params.languages.addimage + '" /></div></div><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.hidelabel_hint + '"><label><input type="checkbox" class="aopts_hide_label ms-checkbox" value=""><span>' + sspa_params.languages.hidelabel + '</span></label></div><div class="img-pos-cont"><div class="optscontainer">' + sspa_params.languages.imgpos + '</div><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" checked class="img-align" value=""> ' + sspa_params.languages.imgdef + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="1"> ' + sspa_params.languages.imgtop + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="2"> ' + sspa_params.languages.imgbot + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="3"> ' + sspa_params.languages.imgontop + '</label></div></div><div class="right_aopts"><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.scorehint + '">' + sspa_params.languages.answerscore + ': <input type="number" size="10" maxlength="10" onkeypress="return numbersandminusonly(event)" placeholder="' + sspa_params.languages.number + '" class="aopts_score optsinput" value="1"></div><div class="answer_tooltip optscontainer"><span>' + sspa_params.languages.customtooltip + '</span><textarea maxlength="300" class="answer_tooltip_text" placeholder="' + sspa_params.languages.customtooltipph + '"></textarea></div><div class="answer_redirection_container optscontainer modal_survey_tooltip" title="' + sspa_params.languages.redirecttooltip + '"><span>' + sspa_params.languages.clredirection + '</span><input class="answer_redirection optsinput" placeholder="' + sspa_params.languages.redplaceholder + '" onkeypress="return numbersandminusonly(event)" value=""></div><div class="answer_category_container optscontainer modal_survey_tooltip" title="' + sspa_params.languages.category_tooltip + '"><span>' + sspa_params.languages.category + '</span><input class="answer_category optsinput" placeholder="' + sspa_params.languages.category_placeholder + '" value=""></div><div class="optscontainer"><span class="autocomplete_container modal_survey_tooltip" title="' + sspa_params.languages.autocomplete + '"><label><input type="checkbox" name="autocomplete' + open_unique_id + '" class="autocomplete' + open_unique_id + ' ms-checkbox" value="0"> ' + sspa_params.languages.autocomplete + '</label></span><span class="converttextarea_container modal_survey_tooltip" title="' + sspa_params.languages.converttextarea + '"><label><input type="checkbox" name="converttextarea' + open_unique_id + '" class="converttextarea' + open_unique_id + ' ms-checkbox" value="0"> ' + sspa_params.languages.converttextarea + '</label></span></div><input type="hidden" class="aopts_status" value="0"></div></div></div>');
		create_graph( answer_area.replace( "answers_", "" ), question_num, "true" );
		initialize_tooltips();
		//surveysystem( "#answer_element_" + answer_area.replace( "answers_", "" ) + "_" + answer_num + " input" ).focus();
		surveysystem( ".oaa" + answer_area.replace( "answers_", "" ) + "_" + answer_num ).accordion({
 	...
 	function add_select_answer( thisanswer ) {
		var answer_area = surveysystem( thisanswer ).parent().parent().attr( "id" );
		var question_num = surveysystem( thisanswer ).parent().parent().parent().attr( "id" ).replace( "question_", "" );
		var answer_num = surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num + " .answer" ).length + 1;
		/*if ( surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num + " .select_answer_style").length > 0 ) {
			showmessage( sspa_params.languages.alreadydatea );
			return;
		}*/
		open_unique_id = ( Math.floor( Math.random() * 26 ) + Date.now() );
		surveysystem( "#" + answer_area.replace( "answers_", "" ) + " #question_" + question_num ).append( '<div class="added_answers" id="answer_element_' + answer_area.replace( "answers_", "" ) + '_' + answer_num + '"><a href="#" class="answer_options" data-aid="' + answer_num + '"><img class="modal_survey_tooltip" title="' + sspa_params.languages.answeroptions + '" src="' + sspa_params.plugin_url + '/templates/assets/img/options.png"></a><a href="#" class="answer_status answer_status' + answer_num + '" data-aid="' + answer_num + '"><img class="modal_survey_tooltip" title="' + sspa_params.languages.activeanswer + '" src="' + sspa_params.plugin_url + '/templates/assets/img/active.png"></a><span class="ans_nmo">'+answer_num+'.</span> ' + sspa_params.languages.answer + ': <input type="text" data-answertype="select" data-unique="' + open_unique_id + '" id="answer'+answer_num+'" name="answer[]" class="answer select_answer_style" value="' + sspa_params.languages.selectanswer + '" /><span id="answer_count'+answer_num+'" class="answer_count" contenteditable="true">0 - 0%</span><a class="remove_answer" id="remove_'+answer_area+'_'+answer_num+'"><img class="modal_survey_tooltip" title="' + sspa_params.languages.removeanswer + '" src="'+sspa_params.plugin_url+'/templates/assets/img/icon-minus.png"></a><div class="ao-accordion oaa' + answer_num + ' oaa' + answer_area.replace( "answers_", "" ) + '_' + answer_num + '"><h4>' + sspa_params.languages.answeroptions + '</h4><div><div class="answer_title">' + sspa_params.languages.answer_select_title + '</div><div class="left_aopts"><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.imgwidthhint + '">' + sspa_params.languages.imgwidth + ': <input type="text" size="10" maxlength="10" placeholder="' + sspa_params.languages.imgwidthp + '" class="aopts_width optsinput" value=""></div><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.imgheighthint + '">' + sspa_params.languages.imgheight + ': <input type="text" size="10" maxlength="10" placeholder="' + sspa_params.languages.imgheightp + '" class="aopts_height optsinput" value=""></div><div class="imageelement"><div class="uploaded_image"><input class="answer-image-upload button add-button" type="button" value="' + sspa_params.languages.addimage + '" /></div></div><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.hidelabel_hint + '"><label><input type="checkbox" class="aopts_hide_label ms-checkbox" value=""><span>' + sspa_params.languages.hidelabel + '</span></label></div><div class="img-pos-cont"><div class="optscontainer">' + sspa_params.languages.imgpos + '</div><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" checked class="img-align" value=""> ' + sspa_params.languages.imgdef + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="1"> ' + sspa_params.languages.imgtop + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="2"> ' + sspa_params.languages.imgbot + '</label><label><input type="radio" name="img-align' + question_num + '_' + answer_num + '" class="img-align" value="3"> ' + sspa_params.languages.imgontop + '</label></div></div><div class="right_aopts"><div class="modal_survey_tooltip optscontainer" title="' + sspa_params.languages.scorehint + '">' + sspa_params.languages.answerscore + ': <input type="number" size="10" maxlength="10" onkeypress="return numbersandminusonly(event)" placeholder="' + sspa_params.languages.number + '" class="aopts_score optsinput" value="1"></div><div class="answer_tooltip optscontainer"><span>' + sspa_params.languages.customtooltip + '</span><textarea maxlength="300" class="answer_tooltip_text" placeholder="' + sspa_params.languages.customtooltipph + '"></textarea></div><div class="answer_redirection_container optscontainer modal_survey_tooltip" title="' + sspa_params.languages.redirecttooltip + '"><span>' + sspa_params.languages.clredirection + '</span><input class="answer_redirection optsinput" placeholder="' + sspa_params.languages.redplaceholder + '" onkeypress="return numbersandminusonly(event)" value=""></div><div class="answer_category_container optscontainer modal_survey_tooltip" title="' + sspa_params.languages.category_tooltip + '"><span>' + sspa_params.languages.category + '</span><input class="answer_category optsinput" placeholder="' + sspa_params.languages.category_placeholder + '" value=""></div><div class="select-values optscontainer"><span>' + sspa_params.languages.listvalues + '</span><textarea class="selectanswer_values optsinput" placeholder="' + sspa_params.languages.selectvalues + '"></textarea></div><input type="hidden" class="aopts_status" value="0"></div></div></div>');
		create_graph( answer_area.replace( "answers_", "" ), question_num, "true" );
		initialize_tooltips();
		//surveysystem( "#answer_element_" + answer_area.replace( "answers_", "" ) + "_" + answer_num + " input" ).focus();
		surveysystem( ".oaa" + answer_area.replace( "answers_", "" ) + "_" + answer_num ).accordion({
  	...
  	surveysystem("body").on( "click", ".duplicate_question",function() {
		...
		surveysystem( duplicated ).find( ".question-area" ).children( "textarea" ).attr( "id", "question" + question_next );
		surveysystem( duplicated ).find( ".left_qopts .img-align" ).attr( "name", "img-align" + question_next );
		...
		surveysystem( duplicated ).find( '.ao-accordion' ).each( function( index, value ) {
			surveysystem( this ).find( '.img-pos-cont .img-align' ).attr( "name", "img-align" + question_next + "_" + index );
		})
	...
	function getFormattedDate(date) {
 	...
 	surveysystem("body").on( "click", ".save_survey",function() {
		if ( rmdni == false ) {
			var buttonspan = surveysystem( this ).parent(), error = false, numeric_min = "", numeric_max = "", numeric_step = "", checker, answers_array = [], cond_array = [], qsc_cond_array = "", qscexp = "";
			...
			if ( error == false ) {
				...
				var data = {
					...
					survey_name: surveysystem(".wrap .survey-title").text()
					};
					checker = setTimeout( function() {
      ...
      buttons: [
		{
			...
			click : function() {
				if ( surveysystem( ".removefromresults" ).prop('checked') ) {
					remove_votes();
				}
				else {
    ...
    surveysystem( "#dialog-confirm8" ).dialog({
     ...
     surveysystem( "#dialog-confirm11" ).dialog({
      resizable: false,
      height: 250,
	  width: 600,
	  autoOpen: false,
      ...
      buttons: [
		{
			text: sspa_params.languages.revert,
			click : function() {
				default_options();
				surveysystem( this ).dialog( "close" );
			}
	...
	surveysystem( "#dialog-help4" ).dialog({
      ...
      buttons: [
		...
		{
			text: sspa_params.languages.addanswer_numeric,
			click : function() {
				var this2 = surveysystem( "#dialog-help4" ).data( 'this' )
				add_numeric_answer( this2 );
				surveysystem( this ).dialog( "close" );
			}
		},
		{
			text: sspa_params.languages.addanswer_date,
			click : function() {
				var this2 = surveysystem( "#dialog-help4" ).data( 'this' )
				add_date_answer( this2 );
				surveysystem( this ).dialog( "close" );
			}
		},
		{
			text: sspa_params.languages.addanswer_select,
			click : function() {
				var this2 = surveysystem( "#dialog-help4" ).data( 'this' )
				add_select_answer( this2 );
				surveysystem( this ).dialog( "close" );
			}
		},
		{
		...
		surveysystem( '.participants-select' ).each( function( index ) {
			if ( surveysystem( this ).val() == '1' ) {
				...
				if ( typeof thisid[ 1 ] != undefined && typeof thisid[ 2 ] != undefined ) {
					remoparts.push( thisid[ 1 ] + '-' + thisid[ 2 ] );
				}
			}
		});
		if ( typeof remoparts[ remocounter ] != "undefined" ) {
			var data = {
				action: 'ajax_survey',
				sspcmd: 'remove_results',
				part: remoparts[ remocounter ],
...
function create_graph( survey_id, question_id, animation ) {
	...
	if ( animation == 'true' ) {
			...
			modalSurveyChart = new MSChart(
				document.getElementById( "modal_survey_pro_graph_" + survey_id + '_' + question_id ).getContext( "2d" ), chartconfig );
		}
	}
	else {
		if ( document.getElementById( "modal_survey_pro_pdf_container_" + survey_id + '_' + question_id ) != undefined ) {
			...
			modalSurveyChart = new MSChart(
				document.getElementById( "modal_survey_pro_pdf_container_" + survey_id + '_' + question_id ).getContext( "2d" ), chartconfig );
...
jQuery(document).ready( function () {
	if ( jQuery( ".survey_theme" ).length > 0 ) {		
		if ( jQuery( ".survey_theme" ).val() != "" ) {
			jQuery( ".style-theme-specific" ).addClass( "dnone" );
		}
		else {
			jQuery( ".style-theme-specific" ).removeClass( "dnone" );			
		}
	}
